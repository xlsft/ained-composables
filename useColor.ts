...
export const useColor = (): {
    ...
    modify: {
        hue: (hex: string, add: number) => string,
        saturation: (hex: string, add: number) => string,
        lightness: (hex: string, add: number) => string
    }
} => {
    const color = {
        convert: {
            ...
            rgb: {
                hex: ({ r, g, b }: { r: number, g: number, b: number }) =>  "#" + (1 << 24 | r << 16 | g << 8 | b).toString(16).slice(1),
                hsl: ({ r, g, b }:  { r: number, g: number, b: number }) => {
                    r /= 255; g /= 255; b /= 255;
            ...
            hsl: {
                rgb: ({ h, s, l }: { h: number, s: number, l: number }) => {
                    s /= 100;
                    l /= 100;
        ...
        modify: {
            ...
            saturation: (hex: string, add: number): string => {
                ...
                else s = Math.max(s + add, 100)
                return color.convert.hsl.hex({ h, s, l })
            },
            lightness: (hex: string, add: number): string => {
                let { h, s, l } = color.convert.hex.hsl(hex)
                if (h + add <= 0) h = 0
                else if (h + add >= 100) h = 100
                else h = Math.max(h + add, 100)
                return color.convert.hsl.hex({ h, s, l })
            },
        }
    }
